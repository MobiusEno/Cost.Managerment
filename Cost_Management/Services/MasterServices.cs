...
namespace Cost_Management.Services
{
    public class MasterServices
    {
        ...
        #region //統計的搜尋
        //根據分頁以及搜尋來取得資料陣列的方法
        public List<expense_form> GetData1List(ForPaging Paging, string Search, string DateSearch,string projectSearch, string sortOrder)
        {

            //宣告要接受全部搜尋資料的物件
            IQueryable<expense_form> SearchData;
            //判斷搜尋是否為空或Null，用於決定要呼叫取得搜尋資料
            if (String.IsNullOrEmpty(Search) && String.IsNullOrEmpty(DateSearch) && String.IsNullOrEmpty(projectSearch))
            {
                SearchData = GetAllData1List(Paging);
            }
            else
            {
                SearchData = GetAllData1List(Paging, Search, DateSearch,projectSearch);
            }
            //先排序再根據分頁來回傳所需部分的資料陣列
            switch (sortOrder)
            {
        ...
        //無搜尋值的搜尋資料方法
        public IQueryable<expense_form> GetAllData1List(ForPaging Paging)
        {
            //宣告要回傳的搜尋資料為資料庫中的Guestbooks資料表
            IQueryable<expense_form> Data = db.expense_form;
            //計算所需的總頁數
        ...
        //包含搜尋值的搜尋資料方法
        public IQueryable<expense_form> GetAllData1List(ForPaging Paging, string Search, string DateSearch, string projectSearch)
        {
            //三個搜尋值都不為空
            if (!String.IsNullOrEmpty(DateSearch) && !String.IsNullOrEmpty(Search) && !String.IsNullOrEmpty(projectSearch))
            {
                IQueryable<expense_form> Data = db.expense_form
               .Where(p => p.userID.Contains(Search) && p.exp_date.Contains(DateSearch) && p.projectCode.Contains(projectSearch));
                //計算所需的總頁數
                Paging.MaxPage = Convert.ToInt32(Math.Ceiling(Convert.ToDouble(Data.Count()) / Paging.ItemNum));
                //重新設定正確的頁數，避免有不正確值傳入
                Paging.SetRightPage();
            ...
            //datesearch=nuull
            else if (!String.IsNullOrEmpty(Search) && String.IsNullOrEmpty(DateSearch) && !String.IsNullOrEmpty(projectSearch))
            {
                //根據搜尋值來搜尋資料
                IQueryable<expense_form> Data = db.expense_form
                    .Where(p => p.userID.Contains(Search)&& p.projectCode.Contains(projectSearch));
                //計算所需的總頁數
                Paging.MaxPage = Convert.ToInt32(Math.Ceiling(Convert.ToDouble(Data.Count()) / Paging.ItemNum));
                //重新設定正確的頁數，避免有不正確值傳入
                Paging.SetRightPage();
            ...
            {
                IQueryable<expense_form> Data = db.expense_form
              .Where(p => p.userID.Contains(Search) && p.exp_date.Contains(DateSearch));
                //計算所需的總頁數
                Paging.MaxPage = Convert.ToInt32(Math.Ceiling(Convert.ToDouble(Data.Count()) / Paging.ItemNum));
                //重新設定正確的頁數，避免有不正確值傳入
                Paging.SetRightPage();
            ...
            {
                IQueryable<expense_form> Data = db.expense_form
              .Where(p => p.userID.Contains(Search));
                //計算所需的總頁數
                Paging.MaxPage = Convert.ToInt32(Math.Ceiling(Convert.ToDouble(Data.Count()) / Paging.ItemNum));
                //重新設定正確的頁數，避免有不正確值傳入
                Paging.SetRightPage();
            ...
            {
                IQueryable<expense_form> Data = db.expense_form
              .Where(p => p.projectCode.Contains(projectSearch));
                //計算所需的總頁數
                Paging.MaxPage = Convert.ToInt32(Math.Ceiling(Convert.ToDouble(Data.Count()) / Paging.ItemNum));
                //重新設定正確的頁數，避免有不正確值傳入
                Paging.SetRightPage();
            ...
            {
                IQueryable<expense_form> Data = db.expense_form
              .Where(p => p.exp_date.Contains(DateSearch) && p.projectCode.Contains(projectSearch));
                //計算所需的總頁數
                Paging.MaxPage = Convert.ToInt32(Math.Ceiling(Convert.ToDouble(Data.Count()) / Paging.ItemNum));
                //重新設定正確的頁數，避免有不正確值傳入
                Paging.SetRightPage();
